...
import { ProductRecord, UpdateProductForm, CreateProductForm } from "@models/product";
...
interface UpsertProductModalProps {
    // When a product is supplied the modal edits it (update). When omitted it creates a
    // new listing. "Upsert" = insert or update; both share the same two-step wizard.
    product?: ProductRecord;
    onClose: () => void;
    // Fired after a successful create/update so the dashboard can refresh.
    onSuccess?: () => void;
}
        ...
        <>
            ...
            <InputContainer>
                ...
                <AlSaqrMultiImageUpload
                    ...
                    setFieldValue={setFieldValue}
                />
                {touched.images && errors.images && <ErrorText>{errors.images as string}</ErrorText>}
            </InputContainer>
...
const CategoryAndAttributesSection = ({
    ...
    <>
        <InputContainer>
            <AlSaqrFormLabel forInput="productCategoryId">Type of Product:</AlSaqrFormLabel>
            <AlSaqrSelect<ProductCategories>
                name="productCategoryId"
                label="Type of Product"
                options={store.productFeedStore.productCategories.map(pc => ({
                    value: pc.id,
                    label: pc.name,
                }))}
            />
...
const ModalHeader = ({ title, onClose }: { title: string; onClose: () => void }) => (
    <div className="flex items-center justify-between gap-3">
        <h2 className="truncate text-base font-bold text-gray-900 dark:text-white sm:text-lg">{title}</h2>
        <button
            type="button"
            onClick={onClose}
            aria-label="Close"
...
// Owner-only create/edit modal for a selling listing — one two-step wizard for both.
// - Step 1 (Information): title, description, images, price.
// - Step 2 (Category & Attributes): category, tags, attributes.
// Updates are routed through the custom ConfirmModal; the API enforces creator-only
// edits via the bearer token, the UI gating here only confirms before mutating.
const UpsertProductModal = observer(({ product, onClose, onSuccess }: UpsertProductModalProps) => {
    const { userProductsFeedStore, productFeedStore, commonStore, modalStore } = useStore();
    const { updateProduct } = userProductsFeedStore;
    const { createProduct } = productFeedStore;
    ...
    const validate = (values: UpsertProductFormValues) => {
        ...
        if (!values.productCategoryId) errors.productCategoryId = "Category is required.";
        return errors;
    };
    ...
    const buildUpdateForm = (values: UpsertProductFormValues): UpdateProductForm => {
        ...
        if (values.price !== "" && Number(values.price) !== product!.price) {
            form.price = Number(values.price);
            fieldsToUpdate.push("price");
        }
        if ((values.images ?? []).join(",") !== (product!.images ?? []).join(",")) {
            form.images = values.images;
            fieldsToUpdate.push("images");
        }
        if ((values.tags ?? []).join(",") !== (product!.tags ?? []).join(",")) {
            form.tags = values.tags;
            fieldsToUpdate.push("tags");
        }
        if (values.productCategoryId && values.productCategoryId !== product!.productCategoryId) {
            form.productCategoryId = values.productCategoryId;
            fieldsToUpdate.push("productCategoryId");
        }
        if (JSON.stringify(values.attributes ?? {}) !== JSON.stringify(product!.attributes ?? {})) {
            form.attributes = values.attributes;
            fieldsToUpdate.push("attributes");
        }

        return form;
    };

    const openUpdateConfirm = (values: UpsertProductFormValues) => {
        const form = buildUpdateForm(values);
        if (form.fieldsToUpdate.length === 0) {
            toast("No changes to save.", { position: 'top-center' });
            return;
        }
        showModal(
            <ConfirmModal
    ...
    const buildCreateForm = (values: UpsertProductFormValues): CreateProductForm => {
    ...
    return (
        <ModalPortal>
            <ModalBody
                ...
                headerChildren={<ModalHeader title={isEdit ? "Edit listing" : "Create listing"} onClose={onClose} />}
            >
                <div className="flex w-full flex-col">
                    {/* Step indicator */}
                    <div className="mb-4 flex flex-wrap gap-2 text-xs font-semibold">
                        {[EditStep.Information, EditStep.CategoryAndAttributes].map((s) => (
                            <span
                                key={s}
                    ...
                    <Formik
                        ...
                        onSubmit={async (values) => {
                            if (isEdit) openUpdateConfirm(values);
                    ...
                    >
                        {({ values, errors, touched, setFieldValue, setTouched, isSubmitting, handleSubmit }) => {
                            ...
                            const goNext = () => {
                                ...
                                if (hasStep1Error) {
                                    setTouched(
                                        { ...touched, title: true, description: true, images: true },
                                        true
                                    );
                                    return;
                                }
                            ...
                            return (
                                <form onSubmit={handleSubmit} className="flex w-full flex-col gap-3" data-testid="upsertproductform">
                                    ...
                                    <div className="mt-2 flex justify-between">
                                        ...
                                        {step === EditStep.Information ? (
                                            ...
                                            <button
                                                ...
                                                className="rounded-full bg-[#55a8c2] px-5 py-2 font-bold text-white disabled:opacity-40"
                                            >
                                                {isEdit ? "Save changes" : isSubmitting ? "Creating…" : "Create listing"}
                                            </button>
                                        )}
                                    </div>
                                </form>
