...
export default class ToysAndGamesFeedStore {
    ...
    constructor() {
        ...
        makePersistable(this, { name: 'ToysAndGamesFeedStore', properties: ['toysAndGamesRegistry'], storage: window.localStorage });
    ...
    loadToysAndGames = async () => {
        ...
        try {
            const { items, pagination } = await commonAgent.productApiClient.getNearbyProductsOnCategory(this.axiosParams, ProductCategories.ToysAndGames) ?? [];
