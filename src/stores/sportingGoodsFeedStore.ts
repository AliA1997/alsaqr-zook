...
export default class SportingGoodsFeedStore {
    ...
    constructor() {
        ...
        makePersistable(this, { name: 'SportingGoodsFeedStore', properties: ['sportingGoodsRegistry'], storage: window.localStorage });
    ...
    loadSportingGoods = async () => {
        ...
        try {
            const { items, pagination } = await commonAgent.productApiClient.getNearbyProductsOnCategory(this.axiosParams, ProductCategories.SportingGoods) ?? [];
