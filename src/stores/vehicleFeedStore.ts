...
export default class VehicleFeedStore {

    constructor() {
        makeAutoObservable(this);
        makePersistable(this, { name: 'VehicleFeedStore', properties: ['vehiclesRegistry'], storage: window.localStorage });
    ...
    vehiclesRegistry: Map<number, ProductRecord> = new Map<number, ProductRecord>();
    ...
    setVehicle = (vehicleId: number, vehicle: ProductRecord) => {
        this.vehiclesRegistry.set(vehicleId, vehicle);
    }
    ...
    loadVehicles = async () => {
        ...
        try {
            const { items, pagination } = await commonAgent.productApiClient.getNearbyProductsOnCategory(this.axiosParams, ProductCategories.Vehicles) ?? [];

            runInAction(() => {
                items.forEach((product: ProductRecord) => {
                    this.setVehicle(product.id, product);
                });
    ...
    get nearbyVehicles() {
        return Array.from(this.vehiclesRegistry.values());
    }
}